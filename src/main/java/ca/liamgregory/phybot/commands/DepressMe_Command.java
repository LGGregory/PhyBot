 ...
 * @author Liam Gregory
 */
public class DepressMe_Command implements Generic_Command {

    @Override
    public void execute(MessageReceivedEvent evt, String... args) {

        evt.getChannel().sendMessage(Ref.depressingTruths.get(Ref.getRng().nextInt(Ref.depressingTruths.size())))
                .queue();
